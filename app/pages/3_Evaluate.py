...
current = cfg.get("eval") or cfg.get("evaluate") or {}
finetune_cfg = cfg.get("finetune") or {}

supported_suites = ["LoRA", "QLoRA", "Full"]

current_suite = current.get("suite")
if current_suite not in supported_suites:
    finetune_method = finetune_cfg.get("method", "LoRA")
    current_suite = "Full" if finetune_method in ["Full", "Partial"] else finetune_method
    if current_suite not in supported_suites:
        current_suite = "LoRA"
...
st.number_input("Max sequence length", min_value=64, max_value=32768, value=inherited_max_seq_len, step=64, disabled=True)
...
use_max_seq_len_override = st.checkbox("Override max sequence length for evaluation", value=use_max_seq_len_override_default)
