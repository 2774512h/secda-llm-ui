...
class FullEval:
    def run(self, config, run: RunPaths, sft_artifact: Dict[str, Any]) -> Dict[str, Any]:
        ...
        model_dir = sft_artifact["model_dir"]
        t0 = time.time()
        ...
        append_log(run, f"[EVAL][Full] device={device.type}")

        append_log(run, f"[EVAL][Full] Loading tokenizer from finetuned dir: {model_dir}")
        tokenizer = AutoTokenizer.from_pretrained(model_dir, use_fast=True)
        if tokenizer.pad_token is None:
            tokenizer.pad_token = tokenizer.eos_token

        append_log(run, f"[EVAL][Full] Loading finetuned model from: {model_dir}")
        ...
        append_log(run, f"[EVAL][Full] Dataset: {dataset_path} (n={len(examples)})")

        loss_metrics = eval_teacher_forced_loss(model, tokenizer, examples, max_seq_len, batch_size, device)
        append_log(run, f"[EVAL][Full] eval_loss={loss_metrics['eval_loss']:.4f}")

        gen_metrics = eval_generation(model, tokenizer, examples, device, max_new_tokens)
        append_log(run, f"[EVAL][Full] gen_exact_match={gen_metrics['gen_exact_match']:.3f}")
