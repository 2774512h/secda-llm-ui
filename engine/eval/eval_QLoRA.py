...
class QLoRAEval:
    def run(self, config, run: RunPaths, sft_artifact: Dict[str, Any]) -> Dict[str, Any]:
        ...
        append_log(run, f"[EVAL][QLoRA] device={device.type}")

        append_log(run, f"[EVAL][QLoRA] Loading tokenizer: {base_model}")
        tokenizer = AutoTokenizer.from_pretrained(base_model, use_fast=True)
        if tokenizer.pad_token is None:
            tokenizer.pad_token = tokenizer.eos_token

        # Load quantized base model (4-bit) then apply adapter
        use_cuda = torch.cuda.is_available()
        use_bf16 = bool(use_cuda and torch.cuda.is_bf16_supported())
        compute_dtype = torch.bfloat16 if use_bf16 else torch.float16

        append_log(run, f"[EVAL][QLoRA] Loading 4-bit base model: {base_model}")
        ...
        append_log(run, f"[EVAL][QLoRA] Loading adapter: {adapter_dir}")
        ...
        append_log(run, f"[EVAL][QLoRA] Dataset: {dataset_path} (n={len(examples)})")

        loss_metrics = eval_teacher_forced_loss(model, tokenizer, examples, max_seq_len, batch_size, device)
        append_log(run, f"[EVAL][QLoRA] eval_loss={loss_metrics['eval_loss']:.4f}")

        gen_metrics = eval_generation(model, tokenizer, examples, device, max_new_tokens)
        append_log(run, f"[EVAL][QLoRA] gen_exact_match={gen_metrics['gen_exact_match']:.3f}")
