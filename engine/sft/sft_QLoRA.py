...
class QLoRASFT:
    def run(self, config: Dict[str, Any], run: RunPaths) -> Dict[str, Any]:
        ...
        use_4bit = bool(qlora_cfg.get("use_4bit", True))
        ...
        compute_dtype_str = str(qlora_cfg.get("bnb_4bit_compute_dtype", "bf16")).lower()

        if compute_dtype_str == "bf16" and torch.cuda.is_available() and torch.cuda.is_bf16_supported():
        ...
        append_log(run, f"[SFT][QLoRA] Loading 4-bit base model: {base_model}")
        ...
        summary = {
            ...
            "quantization": {
                ...
                "bnb_4bit_quant_type": bnb_4bit_quant_type,
                "bnb_4bit_use_double_quant": bnb_4bit_use_double_quant,
                "bnb_4bit_compute_dtype": str(compute_dtype).replace("torch.", ""),
            },
            "cuda_used": use_cuda,
            "train_runtime_s": dt,
